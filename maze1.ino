...
int16_t mousepath[180][2];  
int16_t** pathFirst; 

int16_t comebakkkval = 0;
...
int16_t* solve(int16_t ** arena , int16_t x, int16_t y, int16_t direction,int16_t run){
    ...
    while(arena[x][y]>=0){
        ...
        if(frontSensor<70 && leftSensor <70 && rightSensor<70){
            if(x==13 && y==0){
                continue;
            }
            else{
...
/*
       ...
       // Serial.print("   kotuwe value 1     ");Serial.println(arena[mousepath[array_index-1][0]][mousepath[array_index-1][1]]);
        if(arena[x][y]==0){
            ...
            if (comebakkkval ==1){
                int16_t dirtemp = comeBackprevious(mousepath,array_index,direction);
                int16_t dirtemp1 = gofrontFast(mousepath,array_index,5-dirtemp);
                int16_t dirtemp3 = comeBackprevious(mousepath,array_index,5-dirtemp1);
                pushcount += 100;
                break;
